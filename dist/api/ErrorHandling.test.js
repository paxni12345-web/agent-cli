...
describe('Custom Error Classes', () => {
    test('APIError should create proper error object', () => {
        const error = new ErrorHandling_1.APIError('Test error', 500, 'TEST_ERROR', true, {
            foo: 'bar',
        });
    ...
    test('ValidationError should default to 400 status', () => {
        const error = new ErrorHandling_1.ValidationError('Invalid input', { field: 'email' });
        expect(error.statusCode).toBe(400);
        expect(error.code).toBe('VALIDATION_ERROR');
        expect(error.details).toEqual({ field: 'email' });
    });
    test('AuthenticationError should default to 401 status', () => {
        const error = new ErrorHandling_1.AuthenticationError();
        expect(error.statusCode).toBe(401);
        expect(error.code).toBe('AUTHENTICATION_ERROR');
        expect(error.message).toBe('Authentication required');
    });
    test('NotFoundError should format message correctly', () => {
        const error = new ErrorHandling_1.NotFoundError('User', { userId: '123' });
    ...
    test('RateLimitError should include retryAfter', () => {
        ...
        const error = new ErrorHandling_1.RateLimitError('Too many requests', retryAfter);
    ...
    test('CircuitBreakerOpenError should include service name', () => {
        const error = new ErrorHandling_1.CircuitBreakerOpenError('payment-service', {
...
describe('Error Logger', () => {
    beforeEach(() => {
        ErrorHandling_1.errorLogger.clear();
    });
    test('should log error with context', () => {
        ...
        ErrorHandling_1.errorLogger.log(error, context, 'error');
        const logs = ErrorHandling_1.errorLogger.getLogs();
    ...
    test('should filter logs by severity', () => {
        ErrorHandling_1.errorLogger.log(new Error('Error 1'), { timestamp: new Date() }, 'error');
        ErrorHandling_1.errorLogger.log(new Error('Warning 1'), { timestamp: new Date() }, 'warning');
        ErrorHandling_1.errorLogger.log(new Error('Critical 1'), { timestamp: new Date() }, 'critical');
        const errorLogs = ErrorHandling_1.errorLogger.getLogs({ severity: 'error' });
        expect(errorLogs).toHaveLength(1);
        const criticalLogs = ErrorHandling_1.errorLogger.getLogs({ severity: 'critical' });
        expect(criticalLogs).toHaveLength(1);
    });
    test('should limit log size', () => {
        for (let i = 0; i < 1100; i++) {
            ErrorHandling_1.errorLogger.log(new Error(`Error ${i}`), { timestamp: new Date() }, 'error');
        }
        const logs = ErrorHandling_1.errorLogger.getLogs();
    ...
    test('should sanitize stack traces', () => {
        ...
        ErrorHandling_1.errorLogger.log(error, { timestamp: new Date() }, 'error');
        const logs = ErrorHandling_1.errorLogger.getLogs();
...
describe('Retry Handler', () => {
    test('should succeed on first attempt', async () => {
        let attempts = 0;
        const result = await ErrorHandling_1.RetryHandler.execute(async () => {
            attempts++;
            return 'success';
        });
    ...
    test('should throw after max attempts', async () => {
        let attempts = 0;
        await expect(ErrorHandling_1.RetryHandler.execute(async () => {
            attempts++;
            throw new Error('Persistent failure');
        }, { maxAttempts: 3, initialDelay: 10 })).rejects.toThrow('Persistent failure');
        expect(attempts).toBe(3);
    });
    test('should not retry non-retryable errors', async () => {
        ...
        await expect(ErrorHandling_1.RetryHandler.execute(async () => {
            attempts++;
            throw new ErrorHandling_1.ValidationError('Not retryable');
        }, { maxAttempts: 3, initialDelay: 10 })).rejects.toThrow('Not retryable');
    ...
    test('should use exponential backoff', async () => {
        ...
        try {
            await ErrorHandling_1.RetryHandler.execute(async () => {
                const now = Date.now();
                if (delays.length > 0) {
                    delays.push(now - lastTime);
                }
                lastTime = now;
                throw new ErrorHandling_1.ServiceUnavailableError();
...
describe('Circuit Breaker', () => {
    test('should start in CLOSED state', () => {
        const breaker = new ErrorHandling_1.CircuitBreaker('test-service');
        expect(breaker.getState()).toBe(ErrorHandling_1.CircuitState.CLOSED);
    });
    test('should open after failure threshold', async () => {
        ...
        expect(breaker.getState()).toBe(ErrorHandling_1.CircuitState.OPEN);
    });
    test('should reject requests when OPEN', async () => {
        ...
        await expect(breaker.execute(async () => 'success')).rejects.toThrow(ErrorHandling_1.CircuitBreakerOpenError);
    });
...
describe('Circuit Breaker Manager', () => {
    test('should create and retrieve circuit breakers', () => {
        const manager = ErrorHandling_1.circuitBreakerManager;
...
describe('Error Metrics Collector', () => {
    beforeEach(() => {
        ErrorMiddleware_1.errorMetricsCollector.clear();
    });
    test('should record error metrics', () => {
        const error = new ErrorHandling_1.ValidationError('Test error');
        const context = {
            path: '/api/test',
            timestamp: new Date(),
        };
        ErrorMiddleware_1.errorMetricsCollector.record(error, context);
        const metrics = ErrorMiddleware_1.errorMetricsCollector.getMetrics();
    ...
    test('should filter metrics by error type', () => {
        ErrorMiddleware_1.errorMetricsCollector.record(new ErrorHandling_1.ValidationError('Test'), { path: '/api/test', timestamp: new Date() });
        ErrorMiddleware_1.errorMetricsCollector.record(new ErrorHandling_1.NotFoundError('Resource'), { path: '/api/test', timestamp: new Date() });
        const validationMetrics = ErrorMiddleware_1.errorMetricsCollector.getMetrics({
...
describe('API Gateway Error Handling Integration', () => {
    ...
    test('should handle validation errors', async () => {
        ...
        gateway.registerEndpoint({
            ...
            handler: async () => {
                throw new ErrorHandling_1.ValidationError('Invalid data', { field: 'email' });
            },
            middleware: [],
    ...
    test('should execute with retry', async () => {
        ...
        let attempts = 0;
        const result = await gateway.executeWithRetry(async () => {
            attempts++;
            if (attempts < 2)
    ...
    test('should execute with circuit breaker', async () => {
        const gateway = new APIGateway_1.APIGateway();
        const result = await gateway.executeWithCircuitBreaker('test-service', async () => 'success');
        expect(result).toBe('success');
    });
...
describe('Error Response Format', () => {
    test('should format error response correctly', () => {
        ...
        const gateway = new APIGateway_1.APIGateway();
        const response = gateway.createErrorResponseFromAPIError(error, context);
        expect(response.statusCode).toBe(400);
    ...
    test('should sanitize stack traces', () => {
        ...
        const gateway = new APIGateway_1.APIGateway();
        const sanitized = gateway.sanitizeStackTrace(error.stack);
