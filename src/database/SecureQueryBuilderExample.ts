...
async function exampleSecureQueries() {
  ...
  try {
    ...
    console.error('✗ Query timed out:', (error as Error).message);
  }
  ...
  try {
    const result = await manager.query(
      ...
      ['user@example.com'],
      { prepare: true, cache: true }
    );
    console.log('✓ Prepared statement executed');
  } catch (error) {
    console.error('✗ Prepared statement failed:', (error as Error).message);
  }
...
async function exampleSQLInjectionPrevention() {
  ...
  try {
    ...
    await manager.query(
      'main-db',
      `SELECT * FROM ${tableName}`, // Dynamic identifier
      []
    );
    console.log('✗ Should have been blocked!');
  } catch (error) {
  ...
  try {
    await manager.query(
      'main-db',
      'SELECT * FROM admin_secrets WHERE id = $1',
      [1]
    );
    console.log('✗ Should have been blocked!');
  } catch (error) {
  ...
  try {
    ...
    await manager.query(
      ...
      [userInput] // Safely parameterized
    );
    console.log('✓ Query executed safely (no injection)');
  } catch (error) {
    console.log('Query failed (safe):', (error as Error).message);
  }
...
export {
  setupSecureDatabase,
  exampleSecureQueries,
  exampleSQLInjectionPrevention,
  exampleQueryMonitoring,
