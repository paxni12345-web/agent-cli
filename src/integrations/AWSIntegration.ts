...
export class AWSIntegration extends EventEmitter {
  private config: AWSIntegrationConfig;
  ...
  constructor(config?: Partial<AWSIntegrationConfig>) {
    super();
  ...
  private mergeConfig(config?: Partial<AWSIntegrationConfig>): AWSIntegrationConfig {
    return {
      enabled: true,
      mode: 'production',
      timeout: 30000,
