...
export class DockerManager extends EventEmitter {
  ...
  private mergeConfig(config?: Partial<DockerManagerConfig>): DockerManagerConfig {
    return {
      enabled: true,
      mode: 'production',
      timeout: 30000,
  ...
  public async initialize(): Promise<void> {
    ...
    this.log('info', 'Initializing DockerManager...');
    ...
    this.log('info', 'DockerManager initialized successfully');
  }
  ...
  private log(level: string, message: string): void {
    ...
    if (messageLevelIndex >= currentLevelIndex) {
      if (this.config.debug) {
        console.log(`[${level.toUpperCase()}] [DockerManager] ${message}`);
      }
