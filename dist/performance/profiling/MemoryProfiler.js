...
class MemoryProfiler extends events_1.EventEmitter {
    ...
    detectLeaks(snapshot) {
        ...
        for (const obj of snapshot.objects) {
            ...
            const previousSizes = previousSnapshots.map(s => s.objects.find(o => o.type === obj.type)?.size || 0);
            ...
            if (rate > 0.2 && growth > 100000) { // Growing more than 20% and > 100KB
                const severity = rate > 0.5 ? 'critical' :
                    rate > 0.4 ? 'high' :
                        rate > 0.3 ? 'medium' : 'low';
                suspects.push({
                    type: obj.type,
     ...
     */
    getLeakRecommendation(type, rate) {
    ...
    analyzeMemoryTrend() {
        ...
        const ssResidual = heapUsages.reduce((sum, y, i) => sum + Math.pow(y - (slope * timePoints[i] + intercept), 2), 0);
