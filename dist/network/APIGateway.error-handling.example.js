...
exports.example1_BasicErrorHandling = example1_BasicErrorHandling;
exports.example2_CircuitBreaker = example2_CircuitBreaker;
exports.example3_RetryWithRecovery = example3_RetryWithRecovery;
exports.example4_ErrorMonitoring = example4_ErrorMonitoring;
exports.example5_CustomErrors = example5_CustomErrors;
exports.example6_GracefulDegradation = example6_GracefulDegradation;
exports.example7_ErrorLogging = example7_ErrorLogging;
exports.example8_ProductionSetup = example8_ProductionSetup;
    ...
    gateway.registerRoute({
        ...
        target: {
            ...
            handler: async (req) => {
                ...
                // Validate ID format
                if (!userId.match(/^\d+$/)) {
                    throw new APIGateway_1.ValidationError('Invalid user ID format', [], {
                        userId,
                ...
                if (!user) {
                    throw new APIGateway_1.NotFoundError(`User ${userId} not found`, { userId });
                }
                return {
                    status: 200,
...
async function example3_RetryWithRecovery() {
    ...
    const result = await APIGateway_1.ErrorRecovery.executeWithRecovery(async () => {
        // Potentially failing operation
        const response = await fetch('http://unreliable-api.com/data');
        if (!response.ok) {
            throw new APIGateway_1.UpstreamError(`API returned ${response.status}`, response.status);
        }
        ...
        shouldRetry: (error) => {
            // Custom retry logic
            if (error instanceof APIGateway_1.UpstreamError) {
                return error.upstreamStatus ? error.upstreamStatus >= 500 : true;
            }
            return APIGateway_1.ErrorHandler.isRetryable(error);
        },
    });
...
async function example4_ErrorMonitoring() {
    ...
    const unsubscribe = APIGateway_1.ErrorMonitor.onError((entry) => {
    ...
    setInterval(() => {
        APIGateway_1.ErrorMonitor.checkThresholds({
    ...
    const stats = APIGateway_1.ErrorMonitor.getErrorStats(Date.now() - 3600000); // Last hour
...
// ============================================================================
class DatabaseError extends APIGateway_1.APIGatewayError {
    constructor(message, query, context) {
        super(message, 500, 'DATABASE_ERROR', true, { ...context, query });
    }
}
class ExternalAPIError extends APIGateway_1.APIGatewayError {
    provider;
...
async function example5_CustomErrors() {
    const gateway = new APIGateway_1.APIGateway({ port: 8080, host: '0.0.0.0' });
    gateway.registerRoute({
        path: '/api/data',
        method: 'GET',
        target: {
            ...
            handler: async (req) => {
                try {
                    ...
                    throw new DatabaseError('Failed to query users table', 'SELECT * FROM users', { originalError: error.message });
                }
            },
        },
    });
    ...
    gateway.registerRoute({
        ...
        target: {
            ...
            handler: async (req) => {
                ...
                const recommendations = await APIGateway_1.ErrorRecovery.gracefulDegrade(
...
async function example8_ProductionSetup() {
    ...
    gateway.registerRoute({
        ...
        middleware: [
            ...
            APIGateway_1.MiddlewareFactory.timeout(5000),
            APIGateway_1.MiddlewareFactory.sizeLimit(1024 * 1024), // 1MB
            APIGateway_1.MiddlewareFactory.errorHandler({ enableRecovery: false }),
        ],
        validation: APIGateway_1.ValidationHelpers.createSecureValidation(userSchema),
        auth: {
            type: 'bearer',
            required: true,
    ...
    // Error monitoring
    APIGateway_1.ErrorMonitor.onError((entry) => {
