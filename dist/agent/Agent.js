...
const ToolCallValidator_js_1 = require("./ToolCallValidator.js");
const ErrorRecoverySystem_js_1 = require("./ErrorRecoverySystem.js");
const ToolPerformanceMonitor_js_1 = require("./ToolPerformanceMonitor.js");
...
class Agent {
     ...
     */
    constructor(provider, toolRegistry, permissions, config) {
        ...
        this.validator = new ToolCallValidator_js_1.ToolCallValidator();
        this.errorRecovery = new ErrorRecoverySystem_js_1.ErrorRecoverySystem();
        this.performanceMonitor = new ToolPerformanceMonitor_js_1.ToolPerformanceMonitor();
    ...
    async run(userMessage) {
        ...
        try {
            ...
            if (this.state.iterationCount >= this.config.maxIterations) {
                throw new index_js_1.AgentError(`Maximum iterations (${this.config.maxIterations}) reached`, 'MAX_ITERATIONS');
            }
     ...
     * @private
     */
    async executeToolWithRetry(toolCall) {
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                ...
                if (this.config.autoRecovery && attempt < maxRetries - 1) {
                    const recoveryAction = this.errorRecovery.recover(result.error || 'Unknown error', currentToolCall, this.state, attempt + 1);
                    if (this.config.debug) {
     ...
     */
    async executeToolWithTimeout(toolCall) {
    ...
    async executeTool(toolCall) {
        ...
        if (this.config.validateToolInputs && tool.inputSchema) {
            const validation = ToolCallValidator_js_1.ToolCallValidator.validate(toolCall, tool.inputSchema);
            if (!validation.valid) {
                if (this.config.debug) {
        ...
        if (this.config.strictToolCalling) {
            const safety = ToolCallValidator_js_1.ToolCallValidator.checkSafety(toolCall);
            if (!safety.safe) {
                if (this.config.debug) {
