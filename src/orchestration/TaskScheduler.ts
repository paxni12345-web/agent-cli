...
export class TaskScheduler extends EventEmitter {
  ...
  private mergeConfig(config?: Partial<TaskSchedulerConfig>): TaskSchedulerConfig {
    return {
      enabled: true,
      mode: 'production',
      timeout: 30000,
  ...
  public async initialize(): Promise<void> {
    ...
    this.log('info', 'Initializing TaskScheduler...');
    ...
    this.log('info', 'TaskScheduler initialized successfully');
  }
  ...
  private log(level: string, message: string): void {
    ...
    if (messageLevelIndex >= currentLevelIndex) {
      if (this.config.debug) {
        console.log(`[${level.toUpperCase()}] [TaskScheduler] ${message}`);
      }
