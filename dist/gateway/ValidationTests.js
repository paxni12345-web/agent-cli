...
xssExamples.forEach(example => {
    const detected = ValidationMiddleware_1.SecurityValidator.detectXSS(example);
...
sqlExamples.forEach(example => {
    const detected = ValidationMiddleware_1.SecurityValidator.detectSQLInjection(example);
...
cmdExamples.forEach(example => {
    const detected = ValidationMiddleware_1.SecurityValidator.detectCommandInjection(example);
    console.log(`  "${example}" => ${detected ? '🚫 BLOCKED' : '✅ ALLOWED'}`);
});
...
pathExamples.forEach(example => {
    const detected = ValidationMiddleware_1.SecurityValidator.detectPathTraversal(example);
    console.log(`  "${example}" => ${detected ? '🚫 BLOCKED' : '✅ ALLOWED'}`);
});
// HTML Sanitization
console.log('\n5. HTML Sanitization:');
const htmlInput = '<script>alert("XSS")</script><p onclick="evil()">Hello</p>';
const sanitized = ValidationMiddleware_1.SecurityValidator.sanitizeHTML(htmlInput);
...
emails.forEach(email => {
    const result = ValidationMiddleware_1.CommonSchemas.email.safeParse(email);
...
phones.forEach(phone => {
    const result = ValidationMiddleware_1.CommonSchemas.phone.safeParse(phone);
...
urls.forEach(url => {
    const result = ValidationMiddleware_1.CommonSchemas.url.safeParse(url);
...
passwords.forEach(password => {
    const result = ValidationMiddleware_1.CommonSchemas.password.safeParse(password);
...
const userSchema = zod_1.z.object({
    ...
    website: ValidationMiddleware_1.CommonSchemas.url.optional(),
});
const testUsers = [
    {
        username: 'john_doe',
...
// ============================================================================
console.log('\n\n=== Request Validator Tests ===\n');
const validator = new ValidationMiddleware_1.RequestValidator();
...
// ============================================================================
console.log('\n\n=== Rate Limiting Tests ===\n');
const rateLimiter = new ValidationMiddleware_1.EndpointRateLimiter();
...
const strictConfig = ValidationMiddleware_1.ValidationPresets.strict();
...
const publicConfig = ValidationMiddleware_1.ValidationPresets.publicAPI();
