...
export class IntegrationTestRunner extends EventEmitter {
  private config: IntegrationTestRunnerConfig;
  ...
  constructor(config?: Partial<IntegrationTestRunnerConfig>) {
    super();
  ...
  private mergeConfig(config?: Partial<IntegrationTestRunnerConfig>): IntegrationTestRunnerConfig {
    return {
      enabled: true,
      mode: 'production',
      timeout: 30000,
  ...
  public async initialize(): Promise<void> {
    ...
    this.log('info', 'Initializing IntegrationTestRunner...');
    ...
    this.log('info', 'IntegrationTestRunner initialized successfully');
  }
  ...
  private log(level: string, message: string): void {
    ...
    if (messageLevelIndex >= currentLevelIndex) {
      if (this.config.debug) {
        console.log(`[${level.toUpperCase()}] [IntegrationTestRunner] ${message}`);
      }
