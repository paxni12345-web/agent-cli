...
describe('ValidationMiddleware', () => {
    ...
    const createMockRequest = (body = {}, query = {}, params = {}) => ({
        id: 'test-123',
        method: 'POST',
        path: '/test',
        headers: {},
    ...
    describe('Schema Validation', () => {
        it('should validate valid data against schema', async () => {
            ...
            const result = await APIGateway_1.ValidationMiddleware.validate(request, { schema });
        ...
        it('should validate number ranges', async () => {
            ...
            const validResult = await APIGateway_1.ValidationMiddleware.validate(validRequest, { schema });
            const invalidResult = await APIGateway_1.ValidationMiddleware.validate(invalidRequest, { schema });
        ...
        it('should validate string length', async () => {
            ...
            const validResult = await APIGateway_1.ValidationMiddleware.validate(validRequest, { schema });
            const tooShortResult = await APIGateway_1.ValidationMiddleware.validate(tooShortRequest, { schema });
            const tooLongResult = await APIGateway_1.ValidationMiddleware.validate(tooLongRequest, { schema });
            expect(validResult.valid).toBe(true);
    ...
    describe('XSS Prevention', () => {
        it('should detect script tags', async () => {
            ...
            const result = await APIGateway_1.ValidationMiddleware.validate(request, {
                preventXSS: true,
            });
            expect(result.valid).toBe(false);
    ...
    describe('Common Schemas', () => {
        it('should validate email format', async () => {
            ...
            for (const email of validEmails) {
                const result = APIGateway_1.CommonSchemas.email.safeParse(email);
        ...
        it('should validate phone numbers', async () => {
            ...
            for (const phone of validPhones) {
                const result = APIGateway_1.CommonSchemas.phone.safeParse(phone);
                expect(result.success).toBe(true);
            }
        ...
        it('should validate URLs', async () => {
            ...
            for (const url of validUrls) {
                const result = APIGateway_1.CommonSchemas.url.safeParse(url);
                expect(result.success).toBe(true);
            }
        ...
        it('should validate username format', async () => {
            ...
            for (const username of validUsernames) {
                const result = APIGateway_1.CommonSchemas.username.safeParse(username);
                expect(result.success).toBe(true);
            }
    ...
    describe('ValidationHelpers', () => {
        it('should create secure validation config', () => {
            const schema = zod_1.z.object({ test: zod_1.z.string() });
            const config = APIGateway_1.ValidationHelpers.createSecureValidation(schema);
        ...
        it('should create rate limit configs', () => {
            const strict = APIGateway_1.ValidationHelpers.createRateLimit.strict();
            expect(strict.maxRequests).toBe(10);
            expect(strict.windowMs).toBe(60000);
            const moderate = APIGateway_1.ValidationHelpers.createRateLimit.moderate();
            expect(moderate.maxRequests).toBe(60);
            const lenient = APIGateway_1.ValidationHelpers.createRateLimit.lenient();
            expect(lenient.maxRequests).toBe(100);
        });
        it('should create custom rate limit', () => {
            const custom = APIGateway_1.ValidationHelpers.createRateLimit.custom(50, 30000, 'sliding_window');
