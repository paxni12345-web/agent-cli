...
export const SQLQueryTool: Tool = {
  name: 'sql_query',
  description:
    'Execute SQL queries on connected databases. Supports SELECT, INSERT, UPDATE, DELETE.',
  input_schema: {
    type: 'object',
    properties: {
      connection_id: {
...
export const InspectSchemaTool: Tool = {
  name: 'inspect_schema',
  description:
    'Inspect database schema including tables, columns, indexes, and relationships',
  input_schema: {
    type: 'object',
    properties: {
      connection_id: {
...
export const MongoQueryTool: Tool = {
  name: 'mongo_query',
  description:
    'Execute MongoDB queries including find, insert, update, delete operations',
  input_schema: {
    type: 'object',
    properties: {
      connection_id: {
...
const connectionRegistry = new Map<string, DatabaseConnection>();
...
function formatTable(rows: any[]): string {
  ...
  const columnWidths = keys.map((key) => {
    const maxLength = Math.max(
      key.length,
      ...rows.slice(0, 10).map((row) => String(row[key] || '').length)
    );
