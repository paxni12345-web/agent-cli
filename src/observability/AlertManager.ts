...
export class AlertManager extends EventEmitter {
  ...
  private mergeConfig(config?: Partial<AlertManagerConfig>): AlertManagerConfig {
    return {
      enabled: true,
      mode: 'production',
      timeout: 30000,
  ...
  public async initialize(): Promise<void> {
    ...
    this.log('info', 'Initializing AlertManager...');
    ...
    this.log('info', 'AlertManager initialized successfully');
  }
  ...
  private log(level: string, message: string): void {
    ...
    if (messageLevelIndex >= currentLevelIndex) {
      if (this.config.debug) {
        console.log(`[${level.toUpperCase()}] [AlertManager] ${message}`);
      }
