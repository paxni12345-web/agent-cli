...
export interface TaskRequirements {
  reasoning?: number; // 0-100 how important reasoning is
  speed?: number;
  costSensitivity?: number; // 0-100, higher = prefer cheaper
...
export class MultiModelOrchestrator {
  ...
  registerModel(name: string, config: ModelConfig): void {
    ...
    eventBus.emitSync(
      ...
      { name, modelName: config.modelName },
  ...
  unregisterModel(name: string): void {
    ...
    eventBus.emitSync(
      'orchestrator.model_unregistered',
      { name },
      'MultiModelOrchestrator'
    );
  }
  ...
  async route(
    ...
    try {
      ...
      // All models failed
      throw new Error(
        `All models failed. Primary: ${error instanceof Error ? error.message : error}`
      );
    }
  }
  ...
  selectModel(
    ...
    const reqs: Required<TaskRequirements> = {
    ...
    for (const [name, config] of this.models) {
      ...
      const reasoningDiff = Math.abs(
        config.capabilities.reasoning - reqs.reasoning
      );
    ...
    const estimatedCost =
      (estimatedTokens.input * selected.config.costPerToken.input +
  ...
  private getSuccessRate(modelName: string): number {
  ...
  private generateReason(
    ...
    if (
      requirements.costSensitivity > 70 &&
