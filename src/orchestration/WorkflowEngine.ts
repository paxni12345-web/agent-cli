...
export class WorkflowEngine extends EventEmitter {
  ...
  private mergeConfig(config?: Partial<WorkflowEngineConfig>): WorkflowEngineConfig {
    return {
      enabled: true,
      mode: 'production',
      timeout: 30000,
  ...
  public async initialize(): Promise<void> {
    ...
    this.log('info', 'Initializing WorkflowEngine...');
    ...
    this.log('info', 'WorkflowEngine initialized successfully');
  }
  ...
  private log(level: string, message: string): void {
    ...
    if (messageLevelIndex >= currentLevelIndex) {
      if (this.config.debug) {
        console.log(`[${level.toUpperCase()}] [WorkflowEngine] ${message}`);
      }
