...
export class ServiceRegistry extends EventEmitter {
  ...
  public deregister(serviceId: string): void {
  ...
  public heartbeat(serviceId: string): void {
    const service = this.services.get(serviceId);
  ...
  public discover(query: ServiceQuery): ServiceInstance[] {
    ...
    if (query.healthy !== undefined) {
      instances = instances.filter(s =>
        query.healthy ? s.health.status === 'passing' : s.health.status !== 'passing'
      );
    }
  ...
  private async runHealthChecks(): Promise<void> {
    for (const service of this.services.values()) {
      if (service.health.checks.length === 0) continue;
  ...
  private async executeHealthCheck(service: ServiceInstance, check: HealthCheck): Promise<void> {
  ...
  private updateServiceHealth(service: ServiceInstance): void {
...
export class CircuitBreakerManager extends EventEmitter {
  ...
  public createBreaker(name: string): CircuitBreaker {
  ...
  private updateResponseTime(breaker: CircuitBreaker, duration: number): void {
...
export class LoadBalancer extends EventEmitter {
  ...
  public addBackend(instance: ServiceInstance, weight: number = 1): Backend {
  ...
  public selectBackend(sessionId?: string): Backend | null {
  ...
  public getStats() {
    return {
      ...
      totalConnections: Array.from(this.backends.values()).reduce(
        (sum, b) => sum + b.connections,
        0
      ),
    };
  }
...
export class ServiceMesh extends EventEmitter {
  ...
  public createProxy(service: ServiceInstance): ServiceProxy {
  ...
  public async route(request: ServiceRequest): Promise<ServiceResponse> {
    // Discover service
    ...
    return this.circuitBreaker.execute(
      ...
      () => this.forwardRequest(backend, request)
    );
  }

  private async forwardRequest(backend: Backend, request: ServiceRequest): Promise<ServiceResponse> {
...
export class APIGateway extends EventEmitter {
  ...
  private findRoute(request: any): Route | null {
  ...
  private matchRoute(route: Route, request: any): boolean {
