...
exports.EditFileTool = exports.WriteFileTool = exports.ReadFileTool = exports.ListFilesTool = void 0;
...
class PathValidator {
    ...
    static sanitizePath(userPath) {
        if (!userPath || typeof userPath !== 'string') {
            throw new index_js_1.WorkspaceError('Invalid path: path must be a non-empty string');
        }
        ...
        try {
            ...
            throw new index_js_1.WorkspaceError('Invalid path: contains malformed URL encoding');
        }
        // Check for dangerous patterns
        for (const pattern of PathValidator.DANGEROUS_PATTERNS) {
            if (pattern.test(decoded)) {
                throw new index_js_1.WorkspaceError(`Invalid path: contains dangerous pattern ${pattern}`);
            }
        }
        // Remove null bytes and control characters
        const sanitized = decoded.replace(/[\x00-\x1f\x7f]/g, '');
        if (sanitized !== decoded) {
            throw new index_js_1.WorkspaceError('Invalid path: contains control characters');
        }
        return sanitized;
    }
    /**
     ...
     */
    static async validateWorkspaceBoundary(targetPath, workspaceRoot) {
        ...
        try {
            ...
            throw new index_js_1.WorkspaceError(`Workspace root is invalid: ${error.message}`);
        }
        try {
            ...
            while (true) {
                ...
                if (parent === pathToCheck) {
                    ...
                    throw new index_js_1.WorkspaceError('Cannot validate path: no parent directory exists');
                }
            ...
            if (error instanceof index_js_1.WorkspaceError) {
                throw error;
            }
            throw new index_js_1.WorkspaceError(`Cannot canonicalize path: ${error.message}`);
        }
        ...
        if (!normalizedTarget.startsWith(normalizedWorkspace)) {
            throw new index_js_1.WorkspaceError('Path is outside workspace boundary');
        }
        ...
        if (relativePath.startsWith('..') || path.isAbsolute(relativePath)) {
            throw new index_js_1.WorkspaceError('Path traversal detected');
        }
     ...
     */
    static async validatePath(userPath, workspaceRoot) {
...
Supports recursive listing with depth control and exclude patterns.`;
    ...
    async execute(input, context) {
        try {
            ...
            const files = await this.listDirectory(fullPath, context.workspaceRoot, recursive, maxDepth, excludePatterns, 0);
            return {
                success: true,
                output: files.join('\n'),
    ...
    async listDirectory(dirPath, workspaceRoot, recursive, maxDepth, excludePatterns, currentDepth) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (recursive) {
                    const subFiles = await this.listDirectory(path.join(dirPath, entry.name), workspaceRoot, recursive, maxDepth, excludePatterns, currentDepth + 1);
    ...
    async validatePath(targetPath, workspaceRoot) {
        ...
        try {
            ...
            throw new index_js_1.ToolError(`Path does not exist: ${path.relative(workspaceRoot, targetPath)}`);
        }
    }
}
...
Supports reading specific line ranges to avoid loading large files entirely.`;
    ...
    async validatePath(targetPath, workspaceRoot) {
        ...
        try {
            ...
            if (!stat.isFile()) {
                throw new index_js_1.ToolError('Path is not a file');
            }
        }
        catch (error) {
            if (error.code === 'ENOENT') {
                throw new index_js_1.ToolError('File does not exist');
            }
            throw error;
        }
    }
