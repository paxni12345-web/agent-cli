...
describe('Custom Error Classes', () => {
    describe('APIGatewayError', () => {
        it('should create error with all properties', () => {
            const error = new ErrorHandling_1.APIGatewayError('Test error', 500, 'TEST_ERROR', true, { key: 'value' }, 'req-123');
        ...
        it('should serialize to JSON', () => {
            const error = new ErrorHandling_1.APIGatewayError('Test', 400, 'TEST', true, { a: 1 }, 'req-1');
    ...
    describe('ValidationError', () => {
        it('should create validation error with correct defaults', () => {
            const error = new ErrorHandling_1.ValidationError('Invalid input', { field: 'email' }, 'req-123');
    ...
    describe('RateLimitError', () => {
        it('should include retryAfter property', () => {
            const error = new ErrorHandling_1.RateLimitError('Too many requests', 60, { limit: 100 }, 'req-123');
    ...
    describe('UpstreamError', () => {
        it('should include upstream service details', () => {
            const error = new ErrorHandling_1.UpstreamError('Service failed', 'api.example.com', 502, 'req-123');
...
describe('StackTraceSanitizer', () => {
    describe('sanitize', () => {
        it('should sanitize passwords', () => {
            ...
            const output = ErrorHandling_1.StackTraceSanitizer.sanitize(input);
  ...
  at func3 (/app/src/other.ts:30:15)`;
            const filtered = ErrorHandling_1.StackTraceSanitizer.filterInternalFrames(stack);
    ...
    describe('getRelevantFrames', () => {
        it('should limit stack trace to specified number of lines', () => {
            ...
            const limited = ErrorHandling_1.StackTraceSanitizer.getRelevantFrames(stack, 5);
...
describe('ErrorLogger', () => {
    let logger;
    beforeEach(() => {
        logger = new ErrorHandling_1.ErrorLogger(100);
    });
    it('should log errors with context', () => {
        const error = new ErrorHandling_1.ValidationError('Test error', undefined, 'req-123');
        const context = {
            requestId: 'req-123',
            method: 'POST',
            path: '/api/test',
    ...
    it('should determine severity correctly', () => {
        const lowError = new ErrorHandling_1.ValidationError('Low', undefined, 'req-1');
        const highError = new ErrorHandling_1.ServiceUnavailableError('High', undefined, 'req-2');
    ...
    it('should return filtered logs', () => {
        ...
        logger.log(new ErrorHandling_1.ValidationError('E1'), context);
        logger.log(new ErrorHandling_1.ServiceUnavailableError('E2'), context);
    ...
    it('should provide error statistics', () => {
        ...
        logger.log(new ErrorHandling_1.ValidationError('E1'), context);
        logger.log(new ErrorHandling_1.ValidationError('E2'), context);
        logger.log(new ErrorHandling_1.ServiceUnavailableError('E3'), context);
...
describe('ErrorResponseFormatter', () => {
    it('should format API Gateway errors', () => {
        const error = new ErrorHandling_1.ValidationError('Invalid input', { field: 'email' }, 'req-123');
        const formatted = ErrorHandling_1.ErrorResponseFormatter.format(error, 'req-123', false);
    ...
    it('should include sanitized stack trace when requested', () => {
        ...
        const formatted = ErrorHandling_1.ErrorResponseFormatter.format(error, 'req-123', true);
    ...
    it('should provide user-friendly messages for generic errors', () => {
        ...
        const formatted = ErrorHandling_1.ErrorResponseFormatter.format(timeoutError, 'req-123', false);
...
describe('RetryStrategy', () => {
    it('should retry on retryable errors', async () => {
        ...
        const result = await ErrorHandling_1.RetryStrategy.executeWithRetry(operation, {
            maxRetries: 3,
            initialDelay: 10,
            maxDelay: 100,
            backoffMultiplier: 2,
    ...
    it('should not retry on non-retryable errors', async () => {
        ...
        const operation = async () => {
            attempts++;
            throw new ErrorHandling_1.ValidationError('Invalid input');
        };
        await expect(ErrorHandling_1.RetryStrategy.executeWithRetry(operation, { maxRetries: 3 })).rejects.toThrow('Invalid input');
    ...
    it('should respect max retries', async () => {
        ...
        const operation = async () => {
            attempts++;
            throw new ErrorHandling_1.TimeoutError('Timeout');
        };
        await expect(ErrorHandling_1.RetryStrategy.executeWithRetry(operation, {
            maxRetries: 2,
            initialDelay: 10,
...
describe('EnhancedCircuitBreaker', () => {
    let circuitBreaker;
    beforeEach(() => {
        circuitBreaker = new ErrorHandling_1.EnhancedCircuitBreaker('test-service', {
            failureThreshold: 3,
            successThreshold: 2,
            timeout: 1000,
            halfOpenRequests: 2,
    ...
    it('should transition to half-open after timeout', async (done) => {
        circuitBreaker = new ErrorHandling_1.EnhancedCircuitBreaker('test', {
            failureThreshold: 1,
            timeout: 100,
            volumeThreshold: 1,
        });
...
describe('ErrorRecoveryManager', () => {
    let recoveryManager;
    beforeEach(() => {
        recoveryManager = new ErrorHandling_1.ErrorRecoveryManager();
    });
    it('should attempt recovery with registered strategies', async () => {
        recoveryManager.registerStrategy(ErrorHandling_1.CacheRecoveryStrategy);
        const error = new ErrorHandling_1.ServiceUnavailableError('Service down');
        const context = {
    ...
    it('should return not recovered when no strategy matches', async () => {
        const error = new ErrorHandling_1.ValidationError('Invalid');
        const context = {};
        const result = await recoveryManager.attemptRecovery(error, context);
