...
// ============================================================================
class AzureIntegration extends events_1.EventEmitter {
    config;
    credential;
    // Service clients
    blobServiceClient;
    cosmosClient;
    serviceBusClient;
    eventHubProducerClients = new Map();
    secretClient;
    keyClient;
    ...
    // Active connections
    serviceBusSenders = new Map();
    serviceBusReceivers = new Map();
    isInitialized = false;
    constructor(config) {
        super();
        this.config = config;
        ...
        if (config.credentials.tenantId && config.credentials.clientId && config.credentials.clientSecret) {
            this.credential = new identity_1.ClientSecretCredential(config.credentials.tenantId, config.credentials.clientId, config.credentials.clientSecret);
        }
        else {
    ...
    async initialize() {
        ...
        try {
            ...
            if (this.config.credentials.storageAccountName && this.config.credentials.storageAccountKey) {
                const sharedKeyCredential = new storage_blob_1.StorageSharedKeyCredential(this.config.credentials.storageAccountName, this.config.credentials.storageAccountKey);
                this.blobServiceClient = new storage_blob_1.BlobServiceClient(`https://${this.config.credentials.storageAccountName}.blob.core.windows.net`, sharedKeyCredential);
            ...
            if (this.config.credentials.serviceBusConnectionString) {
                this.serviceBusClient = new service_bus_1.ServiceBusClient(this.config.credentials.serviceBusConnectionString);
                this.log('Service Bus client initialized');
            }
            // Initialize Key Vault
            if (this.config.credentials.keyVaultUrl) {
                this.secretClient = new keyvault_secrets_1.SecretClient(this.config.credentials.keyVaultUrl, this.credential);
                this.keyClient = new keyvault_keys_1.KeyClient(this.config.credentials.keyVaultUrl, this.credential);
                this.log('Key Vault client initialized');
            }
            // Initialize Container Instances
            if (this.config.credentials.subscriptionId) {
                this.containerInstanceClient = new arm_containerinstance_1.ContainerInstanceManagementClient(this.credential, this.config.credentials.subscriptionId);
                this.log('Container Instance client initialized');
            }
            // Initialize SQL Management
            if (this.config.credentials.subscriptionId) {
                this.sqlManagementClient = new arm_sql_1.SqlManagementClient(this.credential, this.config.credentials.subscriptionId);
            ...
            if (this.config.credentials.cognitiveServicesEndpoint && this.config.credentials.cognitiveServicesKey) {
                ...
                this.textAnalyticsClient = new ai_text_analytics_1.TextAnalyticsClient(this.config.credentials.cognitiveServicesEndpoint, cogCredential);
                this.computerVisionClient = new cognitiveservices_computervision_1.ComputerVisionClient(cogCredential, this.config.credentials.cognitiveServicesEndpoint);
                this.log('Cognitive Services clients initialized');
            }
            // Initialize App Service (Functions)
            if (this.config.credentials.subscriptionId) {
                this.webSiteManagementClient = new arm_appservice_1.WebSiteManagementClient(this.credential, this.config.credentials.subscriptionId);
     ...
     * Create container
     */
    async createContainer(containerName, access) {
        this.ensureInitialized();
        if (!this.blobServiceClient) {
            throw new Error('Blob Storage not configured');
        }
     ...
     */
    async readDocument(databaseId, containerId, documentId, partitionKey) {
        this.ensureInitialized();
        if (!this.cosmosClient) {
            throw new Error('Cosmos DB not configured');
        }
     ...
     */
    async updateDocument(databaseId, containerId, documentId, partitionKey, updates) {
        this.ensureInitialized();
        if (!this.cosmosClient) {
            throw new Error('Cosmos DB not configured');
        }
     ...
     */
    async deleteDocument(databaseId, containerId, documentId, partitionKey) {
        this.ensureInitialized();
        if (!this.cosmosClient) {
            throw new Error('Cosmos DB not configured');
        }
     ...
     */
    async createDatabase(databaseId, throughput) {
        this.ensureInitialized();
        if (!this.cosmosClient) {
            throw new Error('Cosmos DB not configured');
        }
     ...
     */
    async createCosmosContainer(databaseId, containerId, partitionKey) {
        this.ensureInitialized();
        if (!this.cosmosClient) {
            throw new Error('Cosmos DB not configured');
        }
    ...
    getCosmosContainer(databaseId, containerId) {
        if (!this.cosmosClient) {
            throw new Error('Cosmos DB not configured');
        }
    ...
    async deployFunction(options) {
        ...
        try {
            const functionApp = await this.webSiteManagementClient.webApps.createOrUpdate(options.resourceGroupName, options.functionAppName, {
     ...
     */
    async invokeFunction(resourceGroupName, functionAppName, functionName, data) {
        this.ensureInitialized();
        if (!this.webSiteManagementClient) {
            throw new Error('App Service not configured');
        }
     ...
     */
    async listFunctions(resourceGroupName, functionAppName) {
        this.ensureInitialized();
        if (!this.webSiteManagementClient) {
            throw new Error('App Service not configured');
        }
     ...
     */
    async sendServiceBusMessage(options) {
        this.ensureInitialized();
     ...
     */
    async receiveServiceBusMessages(queueName, maxMessages = 10) {
        this.ensureInitialized();
        if (!this.serviceBusClient) {
            throw new Error('Service Bus not configured');
        }
     ...
     */
    async sendEventHubEvents(options) {
        this.ensureInitialized();
        ...
        try {
            ...
            if (!producer) {
                producer = new event_hubs_1.EventHubProducerClient(this.config.credentials.eventHubConnectionString, options.eventHubName);
     ...
     */
    async receiveEventHubEvents(eventHubName, consumerGroup, onEvent, maxWaitTimeInSeconds = 60) {
        this.ensureInitialized();
        if (!this.config.credentials.eventHubConnectionString) {
            throw new Error('Event Hub not configured');
        }
        try {
            const consumer = new event_hubs_1.EventHubConsumerClient(consumerGroup, this.config.credentials.eventHubConnectionString, eventHubName);
     ...
     */
    async createContainerInstance(options) {
        this.ensureInitialized();
        if (!this.containerInstanceClient) {
            throw new Error('Container Instances not configured');
        }
        try {
            const containerGroup = await this.containerInstanceClient.containerGroups.beginCreateOrUpdateAndWait(options.resourceGroupName, options.containerGroupName, {
                location: this.config.region || 'eastus',
                containers: [{
                        ...
                        ports: (options.ports || []).map(port => ({ port, protocol: 'TCP' })),
     ...
     */
    async deleteContainerInstance(resourceGroupName, containerGroupName) {
        this.ensureInitialized();
        if (!this.containerInstanceClient) {
            throw new Error('Container Instances not configured');
        }
     ...
     */
    async getContainerInstanceStatus(resourceGroupName, containerGroupName) {
        this.ensureInitialized();
        if (!this.containerInstanceClient) {
            throw new Error('Container Instances not configured');
        }
     ...
     * Create SQL Database
     */
    async createSqlDatabase(options) {
        this.ensureInitialized();
        if (!this.sqlManagementClient) {
            throw new Error('SQL Management not configured');
        }
        try {
            const database = await this.sqlManagementClient.databases.beginCreateOrUpdateAndWait(options.resourceGroupName, options.serverName, options.databaseName, {
                location: this.config.region || 'eastus',
                sku: {
     ...
     * Delete SQL Database
     */
    async deleteSqlDatabase(resourceGroupName, serverName, databaseName) {
        this.ensureInitialized();
        if (!this.sqlManagementClient) {
            throw new Error('SQL Management not configured');
        }
     ...
     * List SQL Databases
     */
    async listSqlDatabases(resourceGroupName, serverName) {
        this.ensureInitialized();
        if (!this.sqlManagementClient) {
            throw new Error('SQL Management not configured');
        }
        ...
        try {
            ...
            if (options.imageUrl) {
                results = await this.computerVisionClient.analyzeImage(options.imageUrl, {
                    visualFeatures: options.features
                });
            }
            else if (options.imageBuffer) {
                results = await this.computerVisionClient.analyzeImageInStream(() => options.imageBuffer, {
                    visualFeatures: options.features
                });
            }
            else {
...
exports.default = AzureIntegration;
